...
def f_Ra(t, tb, th, A, al):
    ...
    if tb[0] <= t <= tb[1]:
        ...
        Ra = exp(th[5]) + (exp(th[6]) - exp(th[5])) * (t - tb[3]) / (tb[4] - tb[3])
    elif tb[4] <= t <= tb[5]:
        Ra = exp(th[6]) + (exp(th[7]) - exp(th[6])) * (t - tb[4]) / (tb[5] - tb[4])
    elif tb[5] <= t <= tb[6]:
        Ra = exp(th[7]) + (s - exp(th[7])) * (t - tb[5]) / (tb[6] - tb[5])
    elif tb[6] <= t <= tb[7]:
        Ra = s + (exp(th[8]) - s) * (t - tb[6]) / (tb[7] - tb[6])
    else:
        Ra = exp(th[8]) * np.exp(-al*(t-tb[7]))
